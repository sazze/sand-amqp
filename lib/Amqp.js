...
class Amqp extends SandGrain {
  ...
  routingKey(event, application, environment) {
    return Amqp.routingKey.apply(Amqp, arguments);
  }

  static routingKey(event, application, environment) {
    application = application || process.env.SZ_APP_NAME || (global.sand ? global.sand.appName : 'unknown');
    environment = environment || (global.sand ? global.sand.env : 'unknown');
    return new EventsD({environment: environment, appName: application}).getRoutingKey(event);
  }
