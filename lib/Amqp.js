...
class Amqp extends SandGrain {
  ...
  publisher(name, config) {
    ...
    if (_.isUndefined(config) && _.isPlainObject(name)) {
      config = name;

      if (config.routingKey) {
        name = config.routingKey;

      } else if (config.exchange && config.exchange.name) {
        name = config.exchange;

      } else {
        throw new Error('Name is required for initializing a custom publisher!');
      }
    }
...
exports.RpcServer = require('./RpcServer');
